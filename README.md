...
这版不是完整生产系统，但已经不是纯 mock Demo。当前仓库是“真实实时转写 + mock 分析与报告 + 可用 debug/replay”的混合形态。
...
- `vision_service.py` 和 `coaching_service.py` 当前仍然是 mock
...
- 实时 transcript 已经是真实能力，但 insight 还不是
