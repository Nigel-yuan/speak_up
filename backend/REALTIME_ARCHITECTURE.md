...
- 摄像头流由训练页统一持有，布局切换不会停掉录制用的视频轨
- 训练结束时，浏览器会用 `MediaRecorder` 把摄像头视频轨和混合音轨录成 replay 媒体文件
- 混合音轨包括用户麦克风输入，以及 QA 模式下 AI interviewer 的流式提问音频
- 录制结果会先缓存在前端当前会话里，再后台上传后端持久化
