...
右侧不是把“文字稿”和“建议”分成两个区域，而是按时间顺序混排：

- `transcript`
- `coach insight`

这样用户点击任意一条内容，都可以直接把左侧媒体跳到对应时间点。

## 3. 数据来源

## 3.1 媒体文件

训练页结束时，前端会把本地录制的媒体文件上传后端。
...
2. 右侧时间线根据 `currentTimeMs` 计算当前高亮项
3. 当前高亮项会自动滚到可视区域
4. 点击右侧任意 transcript 或 coach 卡片，会直接 seek 到对应时间点
...
- 当前右侧时间线以 transcript 和 coach insight 为主，不额外展示波形或逐帧视觉标注
